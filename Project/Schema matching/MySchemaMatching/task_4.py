...
def make2(cluster_path, cluster_folder_name, dict_synonyms_path, schema_path, format):
    print("Cluster:", cluster_folder_name)

    file_synonyms_path = os.path.join(dict_synonyms_path, cluster_folder_name + format)
...
def make(cluster_path, cluster_folder_name, dict_synonyms_path, schema_path, format):
    print("Cluster:", cluster_folder_name)
...
def make_schema_cluster(datasets_base_path, dict_synonyms_path, schema_path, format):
    for cluster_folder_name in os.listdir(datasets_base_path):
        cluster_path = os.path.join(datasets_base_path, cluster_folder_name)
        make(cluster_path, cluster_folder_name, dict_synonyms_path, schema_path, format)


def make_schema_final(datasets_base_path, dict_synonyms_path, schema_path, format):
    make2(datasets_base_path, "idea_final_schema", dict_synonyms_path, schema_path, format)
